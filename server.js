    ...
    // Define a porta do servidor, usando a variável de ambiente PORT (para deploy)
    // Em produção, process.env.PORT DEVE ser a porta que o Railway injeta.
    // Removendo o fallback para garantir que a porta do Railway seja usada.
    const port = process.env.PORT; // <--- AGORA SEM FALLBACK
    if (!port) {
        console.error("❌ ERRO FATAL: Variável de ambiente PORT não definida. O Railway deve injetá-la.");
        process.exit(1); // Encerra o processo se a porta não for definida
    }
    app.listen(port, () => {
        console.log(`Servidor Express rodando na porta: ${port}`); // Loga a porta real
        console.log(`Ambiente: ${process.env.NODE_ENV}`); // Loga o ambiente para depuração
    });
